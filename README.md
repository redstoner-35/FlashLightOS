...
**注：在锁定模式下，您可通过单击单击并迅速按住按钮(持续1秒以上)让手电进入紧急月光照明模式，而无需进行解锁操作。进入紧急照明月光模式后可通过单击关闭手电，如果您长按按钮则可以调整紧急月光模式的亮度，亮度范围固定为驱动最低电流-极亮电流的5%。**
...
+ [S] : **停止序列执行命令**-当序列执行到此命令后，LED将会强制被设置为熄灭，并且序列停止继续执行直到下次手电重新以此挡位开机。
