...
若执行次数为偶数次，则该命令后面的指令会被继续执行直到序列结束。例如 [1234T5678] 这样的序列。在第1,3,5...(后面以此类推)只会执行
序列前面的四个亮度命令[1234]，后面的内容会被直接跳过。如果是偶数次(2,4,6,8...)则会执行后面的[5678]这四个亮度命令。
...
+ 序列执行频率[4-25(Hz)]：设置自定义闪的序列执行的速度。设置为2Hz则序列每秒钟执行2个指令。对于[1010]这样的序列则会产生
1Hz的闪烁。
