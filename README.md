...
+ 单击并迅速按住按钮(持续2秒以上)：无极调光模式下调整亮度，其他模式和待机状态下会调用电池电压指示器，显示电池电压（如果库仑计已校准则按照百分比显示电池容量）。
+ 双击并迅速按住按钮(持续2秒以上)：启用和禁用自动关机计时器。
+ 四击：进入和退出战术模式
...
+ 自动关机定时器整定值[1-360分钟]：设置该挡位的自动关机时间。当用户通过双击+长按启用自动关机定时器且手电处于工作状态，手
电将会开始按照你设定的时间倒计时。倒计时结束后手电就会自动关机。如果该参数设置为0则该挡位不支持自动关机功能。
