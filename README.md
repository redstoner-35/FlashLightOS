...
+ 自动关机计时器启用提示：当用户通过双击+长按启用了自动关机计时器之后,侧按将会通过指示灯序列提示用户定时器激活以及定时器的
延时时间。此时侧按会以黄色闪烁2次然后紧接着以绿色闪烁2次，停顿1秒后通过一定次数的闪烁提示用户当前自动关机定时器的时间整定值。
对于在60分钟内的时间，侧按将以黄色闪烁，一次代表时间10分钟。超过60分钟的时间则以红色闪烁，一次代表1小时。
+ 自动关机定时器禁用提示: 当用户在定时器启用后双击+长按,则定时器会被关闭。此时侧按将通过黄色闪烁2次然后紧接着以绿色闪烁1次的
序列提示用户。
+ 绿色持续点亮：手电正常运行中，电量充足
