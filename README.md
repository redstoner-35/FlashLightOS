...
当手电指示您可以输入重置密码时，您就可以按照从左到右的顺序输入附赠在手电包装内的重置密码卡上的5位数字，对于默认的重置密码则为`54321`。输入的方式是按照当前所需要输入的数字连续的按压侧按指定次数，例如当前需要输入的数字为5，则快速按压侧按5次。
按压结束后手电将会用红色闪烁来提示您本次输入的数字，例如您按压了5次（5击）则侧按LED将会红色闪烁5次。闪烁结束后1秒，手电将会用黄色闪烁提示您下一个需要输入的数字是密码中的第几个数字，例如闪烁2次则表示您接下来需要输入重置密码中的第二个数字。如果您觉得您输入的该数字有误，则可长按侧按开关直到侧按指示灯以绿色亮起后松开。此时手电将会返回上一位需要输入的数字并通过侧按的黄色闪烁次数指示您需要输入密码中的第几个数字，例如闪烁2次则表示您接下来需要输入重置密码中的第二个数字。
...
+ 绿色持续闪烁：您输入的重置密码正确，系统正在重置设置请稍等。

当指示灯开始以绿色持续闪烁的时候则说明重置进程进行中，此时您需要耐心等待手电完成操作。完成重置操作后手电会自动重启并重新自检，此时您可以使用系统的默认凭据（用户名和密码都是ADMIN）进行登录。
