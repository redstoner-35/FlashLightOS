...
+ 手电降档提示:在手电筒开启的操作途中。如果手电因为电池质量过低触发告警或者温控启动进入降档状态，则侧按LED将会每3秒左右闪烁指定次数以提示您手电已处于降档（功率限制）状态，对应的闪烁规律如下所示。
...
+ 黄色快速闪1次紧接着红色闪一次：鸡血技能（关闭温控并强制手电筒恢复为挡位的额定亮度直到温度达到温控启动点）次数已经用尽暂时无法使用。您需要等待手电筒冷却后才能再次使用此技能。
