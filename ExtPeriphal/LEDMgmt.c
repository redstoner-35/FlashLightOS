...
E=强制结束当前pattern
*/
#define LEDPatternSize 34
const char *LEDPattern[LEDPatternSize]=
 {
   "00",//LED熄灭 0
   "1133220002010DE",//红黄绿分别亮一次然后绿色闪两次指示自检结束 1
	 "111",//绿灯常亮 2
	 "222",//红灯常亮 3 
   ...
   "202020D3030D10DDD",//红色灯闪三次，橙色两次，绿色一次，驱动内部灾难性逻辑错误 17
	 ...
	 "2020D303003030DE",//红色闪2次，暂停1秒然后黄色闪4次，进行重置的时候数据还原失败 32
	 NULL//结束符
 };
//变量
...
extern bool IsInplAlertOccurred; //是否激活输入电流限制
...
static bool CheckForLEDOnStatus(void)	
  {
	...
	else if(SysPstatebuf.CurrentThrottleLevel<4&&!IsInplAlertOccurred&&!ReverseTactalEnabled)return false;//反向战术模式温控和输入低电流警告未介入
	...
	if(CheckForLEDOnStatus())	
	  {
		...
		else if(IsInplAlertOccurred&&(LEDThermalBlinkTimer==4||LEDThermalBlinkTimer==2))IsNeedToOffLED=true; //电池质量警报触发，提示限流发生
