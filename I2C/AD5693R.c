...
//检测DAC是否就绪
bool AD5693R_Detect(void)
  {
	char i;
	//发送起始位，地址，NOP命令和2byte数据，如果都ACK则芯片已就绪
  IIC_Start();
  IIC_Send_Byte(AD5693ADDR);
...
//根据传入的结构体配置DAC的参数	
bool AD5693R_SetChipConfig(DACInitStrDef *DACInitStruct)
  {
	unsigned int buf=0; 
	//根据配置填写对应内容
  ...
  UartPost(msg_error,"DACDrv","Control Register=0x%04X.",buf);
	...
	buf=0;
	return PMBUS_WordReadWrite(true,true,&buf,AD5693ADDR,0x30);//将DAC值POR为0
	}
...
此时DAC输出电压将达不到2倍Vref。*/
bool AD5693R_SetOutput(float VOUT)
  {
	unsigned int vid; 
	//计算VID Code
	float Buf=VOUT;
